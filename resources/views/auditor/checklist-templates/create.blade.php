        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form action="{{ route('auditor.checklist-templates.store') }}" method="POST" class="space-y-6">
                        @csrf

                        <div>
                            <label for="nama_template" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('Nama Template') }}</label>
                            <input type="text" name="nama_template" id="nama_template" value="{{ old('nama_template') }}" required class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm @error('nama_template') border-red-500 @enderror" placeholder="Contoh: Checklist Keamanan">                        
                            @error('nama_template')
                        ...
                        <div class="flex justify-end space-x-4">
                            ...
                            <button type="submit" class="inline-flex justify-center rounded-md border border-transparent bg-indigo-600 py-2 px-4 text-sm font-medium text-white shadow-sm hover:bg-indigo-700">{{ __('Simpan Template') }}</button>
                        </div>
                    </form>
                </div>
            </div>
