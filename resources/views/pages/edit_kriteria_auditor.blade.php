        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-md sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <h1 class="text-2xl font-bold text-gray-800 mb-6">Ubah Kriteria Audit</h1>
                    ...
                    <form action="{{ route('kriteria.update', $kriteria->id) }}" method="POST">
                        ...
                        <div class="space-y-6">
                            <div>
                                ...
                                <input type="text" name="nama_kriteria" id="nama_kriteria" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-emerald-500 focus:border-emerald-500 sm:text-sm" value="{{ old('nama_kriteria', $kriteria->nama_kriteria) }}" required>
                            </div>

                            <div>
                                <label for="deskripsi_kriteria" class="block text-sm font-medium text-gray-700">Deskripsi</label>
                                <textarea name="deskripsi_kriteria" id="deskripsi_kriteria" rows="4" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-emerald-500 focus:border-emerald-500 sm:text-sm" required>{{ old('deskripsi_kriteria', $kriteria->deskripsi_kriteria) }}</textarea>
                            </div>
                        </div>

                        <div class="flex items-center justify-end mt-8 pt-5 border-t border-gray-200">
                            <a href="{{ route('kriteria.index') }}" class="inline-flex items-center px-4 py-2 bg-white border border-gray-300 rounded-md font-semibold text-xs text-gray-700 uppercase tracking-widest shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-emerald-500 disabled:opacity-25 transition ease-in-out duration-150">
                                Batal
                            </a>
